aggregate_data <- function(data, agg_vars, col_so, col_text, col_sm, col_int) {
  ...
  data_agg_so <- data %>%
    ...
    summarise(across(all_of(setdiff(col_so, agg_vars)), Mode, .names = "mode_{.col}"), .groups = "drop")
  
  # 2. Aggregation for select text questions (Mode)
  data_agg_text <- data %>%
    group_by(across(all_of(agg_vars))) %>%
    summarise(across(all_of(col_text), Mode, .names = "mode_text_{.col}"), .groups = "drop")
  ...
  data_agg_sm_first <- data %>%
    group_by(across(all_of(agg_vars))) %>%
    summarise(across(all_of(col_sm), ~ Mode(replace(., . == "do_not_know", NA)), .names = "mode_first_{.col}"), .groups = "drop")
  
  # 5. Aggregation for integer columns (mean)
  data_agg_int <- data %>%
    group_by(across(all_of(agg_vars))) %>%
    summarise(across(all_of(col_int), ~ mean(as.numeric(na.omit(.)), na.rm = TRUE), .names = "mean_{.col}"), .groups = "drop")
