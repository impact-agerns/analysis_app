  ...
  if (nrow(df_filtered)>0){
    ...
    if (sum(df_filtered$count, na.rm=T)>=n_min) {
      ...
      labs(y="% of respondents") + geom_text(aes(y=mean+0.05, label=paste0(round(100*mean, 1), "%")), size=2.5)
        ...
        plot <- plot + geom_text(aes(y=mean+0.02, label=paste0("n=", count)), size=3)
      }
    # if (perc) {plot <- plot + 
  ...
  for (choosen in unique(df$choice)){
    ...
    if (nrow(df_filtered)>0){
      plots[[choosen]] <- df_filtered %>% ggplot(aes(x=reorder(disag_val_1,mean), y=mean))+
        ...
        labs(y="% and number of respondents") + geom_text(aes(y=mean+0.02, label=paste0(round(100*mean,0), "%")), size=2.5)+
        geom_text(aes(y=mean+0.05, label=paste0("(n=", count,")")), size=2.5)
